...
class RiskFlag {
  /// Builds a [RiskFlag] directly — useful for constructing a fake one in tests.
  const RiskFlag({
    required this.id,
    required this.title,
  ...
  /// A stable, machine-readable identifier for this flag, e.g. `'rooted'`.
  final String id;

  /// A short, human-readable summary of the flag.
  final String title;

  /// A longer, plain-language explanation of why this flag matters.
    ...
    if (results[0]) {
      flags.add(
        const RiskFlag(
          ...
          description:
              'Rooted devices can bypass normal app sandboxing protections.',
        ),
      );
    }
    if (results[1]) {
      flags.add(
        const RiskFlag(
          ...
          description: 'Tools like Frida or Xposed can intercept or modify app behavior at runtime.',
        ),
      );
    }
    if (results[2]) {
      flags.add(
        const RiskFlag(
          ...
          description:
              'A debugger can inspect or modify the app while it runs.',
        ),
      );
    }
    if (results[3]) {
    ...
    if (results[4]) {
      flags.add(
        const RiskFlag(
          id: 'vpn',
          title: 'VPN is active',
          description: 'Network traffic is being routed through a third party. This can be legitimate, but can also be used to mask origin.',
        ),
      );
    }
    if (results[5]) {
      flags.add(
        const RiskFlag(
          ...
          description: 'Without a PIN, pattern, or biometric lock, anyone with physical access to the device can use it.',
        ),
      );
    }
    return flags;
